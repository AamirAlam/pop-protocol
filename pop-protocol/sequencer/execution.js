...
function Sequencer(trading_addr, nft_addr) {
    ...
    try {
      ...
      if (events.length) {
        for (const item of events) {
          console.log(
            `${timestamp} MonitorService: MINT FOUND in block ${item.blockNumber}`
          );
        }
      } else {
        console.log(
          `${timestamp} MonitorService: no mints found in block ${web3.eth.blockNumber}`
        );
      }
    } catch (e) {
      console.log(
  ...
  async function request_onchain_price() {
    ...
    const price = await chainlink_oracle_eth_to_usd.methods
      .latestAnswer()
      .call();
  ...
  async function request_offchain_price() {
    const price = await fetchPriceFromCoinGecko("ethereum");
  ...
  async function sequencer_loop() {
    while (true) {
      ...
      console.log(
        `${name} checking for events from ${trading_contract.address}.....`
      );
