const { SourceFileLoader } = require('module');
const { fetch_events, load_contract, coingecko_pricing } = require('eb/pop_protocol/tools');

function Sequencer(web3) {
    const name = "Sequencer";
    const deployer = SourceFileLoader("Deployer", "/home/ice/Projects/eb/pop_protocol/unit_testing/deployer.py").load_module();
    const settlement = SourceFileLoader("Settlement", "/home/ice/Projects/eb/pop_protocol/sequencer/settlement.py").load_module();
    ...
    function deploy_contracts(account) {
        ...
        console.log(`trading contract owner: ${trading_contract.functions.owner().call()}`);
    }

    function check_for_mints() {
        try {
            const events = fetch_events(trading_contract.events.Mint, web3.eth.blockNumber - 50);
            if (events.length > 0) {
                for (const item of events) {
                    console.log(`MonitorService: EVENT FOUND in block ${item.blockNumber}`);
                }
            }
        } catch (e) {
    ...
    function request_onchain_price() {
        ...
        const chainlink_oracle_eth_to_usd = load_contract(web3, oracleABIs.chainlink_eth_to_usd, "0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419");
    ...
    function load_math_obj(n) {
        const load_math = SourceFileLoader("PerpetualOptionsProtocol", "/home/ice/Projects/eb/pop_protocol/math/pop_math.py").load_module();
        const math = load_math.PerpetualOptionsProtocol(n);
        return math;
    }

    function simulate_mint() {
        const math = load_math_obj(1000);
    ...
    function mint_NFT(account) {
        ...
        const tx = settlement.write_NFT_to_chain(account, contract_objects["NFT"], array, array);
...
async function main {
    ...
    const web3 = new Web3(new Web3.providers.HttpProvider(ganache, { timeout: 100 }));
    const privateKey = "0xe0a8f44bae8acd33ed45cc686b25c32fd7b2df98546128aa31e35d57dcadea79";
    const account = web3.eth.accounts.privateKeyToAccount(privateKey);

    const executor = Sequencer(web3);
    executor.deploy_contracts(account);
    executor.check_for_mints();
    executor.price_check();
