...
interface IStaking {
    function updateStaking(address account) external;
    function notifyStakeReceived(uint256 amount) external;
}
contract Pool {
    ...
    address public staking; // contract

    uint256 public maxPop = 1000000 * 10 ** 18;
	...
	function deposit(uint256 amount) external payable {
		...
		require(amount + lastBalance <= maxPop, "!max-pop");
        ...
        IStaking(staking).updateStaking(msg.sender);
	...
	function withdraw(uint256 currencyAmount) external {
		...
		// Send fee to this pool's staking contract
		uint256 feeAmount = currencyAmount;
		_transferOut(staking, feeAmount);
		IStaking(staking).notifyStakeReceived(feeAmount);
