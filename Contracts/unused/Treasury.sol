...
interface IRouter {
    ...
    function popPool() external view returns (address);
    ...
    function getPopShare(address currency) external view returns(uint256);

    function getPoolRewards(address currency) external view returns (address);

    function getPopRewards(address currency) external view returns (address);
}
...
contract Treasury {
	...
	function notifyFeeReceived(
		...
		address popRewards = IRouter(router).getPopRewards(currency);
		...
		// Send popPoolShare to pop-currency rewards contract
		uint256 popReward = IRouter(router).getPopShare(currency) * amount / 10**4;
		_transferOut(currency, popRewards, popReward);
		IRewards(popRewards).notifyRewardReceived(popReward);
