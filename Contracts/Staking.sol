// Pool contract for staking rewards
...
contract Staking {
	...
	function notifyRewardReceived(uint256 amount) external onlyPool {
	...
	function updateRewards(address account) public {
		...
		claimableReward[account] += accountBalance * (cumulativeRewardPerTokenStored - previousRewardPerToken[account]);
	...
	function getClaimableReward() external view returns(uint256) {
		...
		uint256 _pendingReward = pendingReward + ITrading(trading).getPendingFee(currency) / 10**4;

		uint256 _rewardPerTokenStored = cumulativeRewardPerTokenStored + _pendingReward / supply;
		...
		return currentClaimableReward + accountStakedBalance * (_rewardPerTokenStored - previousRewardPerToken[msg.sender]);
